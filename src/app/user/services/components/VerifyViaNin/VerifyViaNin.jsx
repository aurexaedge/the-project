'use client';
import styles from './VerifyViaNin.module.css';
...
const VerifyViaNin = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await axios.get(
        `/api/v1/user?userid=${session.user._id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const resData = await axios.post('/api/v1/verify-nin/v3', {
      //   ninValue,
      //   email: userDetails?.email,
      //   searchVariable: 'nin',
      // });
      ...
      setError(error.response.data.message || 'time out error fetching data');
      setIsLoading(false);
      ...
      <div className={styles.dashboard_container}>
        <div className={styles.wrapper}>
          <p className={styles.message}>
            ...
            You will only be charged 200 NGN per call when there is a search
            result
          </p>
          {/* <span className={styles.info}>
            - Always search twice when you have no record
          </span> */}
          <h1 className={styles.nin_header}>Search via Nin</h1>
