...
export const GET = async (req, { params }) => {
  try {
    ...
    const personalisedData = await personalisationOrderModel
      .findById(orderId)
      .populate({
        path: 'userId',
    ...
    return response(200, personalisedData);
  } catch (error) {
    console.log(error);
...
export const PUT = async (req, { params }) => {
  try {
    ...
    personalisedData.isCompleted = true;
    personalisedData.pdfUrl = pdfUrl;
    personalisedData.message = message;
    await personalisedData.save();
